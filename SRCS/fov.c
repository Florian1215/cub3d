...
/*   Updated: 2023/06/10 17:30:02 by mfinette         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_fov(t_data	*data)
{
	...
	while (angle <= angle_to_radian(data->player.direction + (FOV / 2)) && i < data->distances->size)
	{
		...
		angle += angle_to_radian(DEFINITION);
		i++;
	}
}
